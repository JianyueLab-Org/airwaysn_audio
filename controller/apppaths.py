...
用户解压到哪儿就在哪儿双击，当前目录就是程序目录，`radio_settings.json` 落在 exe
边上，绿色、可搬、看得见。
...
觉得"这软件记不住东西"。所以 macOS 上必须换个地方，按平台惯例是
    ...
    ~/Library/Application Support/audio-for-can/
...
2. **当前目录已经有这个文件，就用那一份。** 从源码跑（`cd controller; python
