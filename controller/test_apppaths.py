"""用户数据目录的测试。
    ...
    python -m unittest test_apppaths -v      （在 controller 目录下运行）
...
class DataDirTest(unittest.TestCase):
    ...
    def test_the_override_wins_everywhere(self):
        ...
        self.assertEqual(apppaths.data_file("radio_settings.json"),
                         os.path.join(target, "radio_settings.json"))
    ...
    def test_the_override_beats_a_file_in_the_working_directory(self):
        ...
        with open("radio_settings.json", "w") as f:
            f.write("{}")
        target = os.path.join(self.folder, "elsewhere")
        os.environ[apppaths.ENV_OVERRIDE] = target
        self.assertEqual(apppaths.data_file("radio_settings.json"),
    ...
    def test_an_existing_file_in_the_working_directory_wins(self):
        ...
        with open("radio_settings.json", "w") as f:
            f.write("{}")
        expected = os.path.join(os.getcwd(), "radio_settings.json")
        for platform in ("win32", "darwin", "linux"):
            with mock.patch.object(sys, "platform", platform):
                self.assertEqual(
                    os.path.realpath(apppaths.data_file("radio_settings.json")),
    ...
    def test_macos_goes_to_application_support(self):
        ...
        with mock.patch.object(sys, "platform", "darwin"):
            path = apppaths.data_file("radio_settings.json")
        self.assertIn(os.path.join("Library", "Application Support",
                                   apppaths.APP_DIR), path)
        self.assertTrue(path.endswith("radio_settings.json"))
