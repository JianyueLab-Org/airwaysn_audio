...
APP_NAME = 'audio-for-can'
BUNDLE_ID = 'org.airwaysn.audio-for-can'
...
if MACOS:
    # 没有 BUNDLE 就只有 dist/audio-for-can/ 一个目录和里面一个可执行文件——
    ...
    app = BUNDLE(
        ...
        info_plist={
            ...
            # **少了这一条，macOS 不是拒绝授权，而是直接把进程杀掉。** 管制端
            # 一打开麦克风就没了，日志停在那一行，看起来像音频库崩了。
            # 这句话是系统弹窗里给用户看的，所以按仓库惯例用中文，后面跟一句
            # 英文——英文界面的用户也看得到同一个弹窗。
            'NSMicrophoneUsageDescription':
                '管制端需要使用麦克风，才能在频率上通话。'
                ' Microphone access is required to transmit on your frequencies.',
        },
    )
