...
**A repeating failure gets one line, and a silent one gets a line at all.** The two halves of the same rule, both learned from one `msfs-for-can` log: it carried **8,553** copies of `failed to play the received audio: [Errno -9988] Stream closed` — one per 20 ms frame — which said nothing 8,552 of those times and pushed every other clue out of the 1 MB rotation, and **40** copies of `could not open joystick 0: …` at one per retry. Meanwhile the paths that were genuinely broken said nothing at all. So:

- **Say it once, in full, then count.** `Voice._note_play_error()` logs a WARNING only when the reason *changes*, carrying the talker, the speaker state and the sample rate; the rest are counted and folded into the burst's own conclusion line (`heard 1003 for 3.1 s (0 frames); 155 frame(s) could not be played (OSError: [Errno -9988] Stream closed)`). That line also now counts unplayable frames toward the duration — without them a 3-second burst that failed printed `0.0 s`, which reads as "nothing arrived" when in fact everything did. `PttWatcher._joystick_loop` does the same with `OPEN_COMPLAIN_EVERY` (one line, then one per ~60 s), and **logs the recovery** — a log that only ever fails leaves you unable to tell whether it came back.
- **A path that gives up must say so.** `_open_stick` used to `return None` when the bound device index didn't exist, and an out-of-range button or hat was released in total silence — so "PTT doesn't work" produced a log where nothing at all was wrong. Both now report, once, naming the device and what it actually has (`PTT is bound to button 19 but joystick 0 (WINCTRL URSA MINOR) only has 4 button(s)`).
- **Name the binding, not just the count.** `PTT watcher started with 1 binding(s)` was the real log line, and it does not say whether that one binding is a key or a yoke button. `ptt.describe()` renders bindings for the log in English (the UI wording stays in `i18n.binding_label()` — same string, two audiences, deliberately not shared).
- **Reading device names costs an open.** `describe_devices()` opens and closes every joystick to read its name, so it is called only on failure paths; the healthy start-up line reports the device *count* and leaves the user's yoke alone.
- **Keep log literals ASCII.** The console handler on a Chinese Windows is GBK, so an em dash comes out as mojibake or trips `logging` itself. The file handler is UTF-8, but the two have to agree.
...
- **A hat (POV) is its own `kind`, not a button.** On a flight yoke the PTT is usually on a hat (Saitek/Logitech Pro Flight Yoke, Honeycomb Alpha), and SDL reports those through `get_hat()` — `get_button()` never sees them. Without the `HAT` kind the user presses their PTT in 添加绑定 and *nothing happens*, so the dialog just sits at 请按下…, which reads as a broken dialog rather than an unsupported device. A hat gives a `(x, y)` pair, so a binding is a hat index plus one of `HAT_DIRECTIONS`' eight names, and `hat_pressed()` decides the match: a **cardinal** binding compares only its own component (bound "up", pushed up-right → still transmits; going silent exactly on the diagonal is the kind of fault nobody can self-diagnose), while a **diagonal** binding needs both, or "up_right" would be covered by "up" and "right" at once. Buttons and hats share the one poll thread and the one device index, so `_sync_sources()` starts it for either kind and `_clear_stick_state()` clears both down-sets — leaving either one set on a lost device leaves the microphone open. `token()` renders `"0↑"`: the arrow is a symbol, not a word, so it stays in the shared file while `i18n`'s `ptt.hat` supplies the wording.
...
- **`_import_pygame()` must be called from the GUI thread, never from the poll thread.** SDL's DirectInput backend needs an HWND for `SetCooperativeLevel`, and that hidden window is created by `SDL_InitSubSystem(JOYSTICK)` and — per Windows' rules — **owned by the thread that created it**. When that thread exits the system destroys the window, while SDL keeps the stale handle in a global and never rebuilds it. Initialising inside `_joystick_loop`/`PttCapture._joystick_loop` therefore worked exactly once per process: the moment that thread ended — which **opening the settings dialog does**, because `open_settings()` stops the watcher — every subsequent open failed with `IDirectInputDevice8::SetCooperativeLevel() DirectX error 0x80070006` (`E_HANDLE`) until the client was restarted. A real `msfs-for-can` v2.0.3 log shows the whole arc: the user records their yoke button successfully at 16:56:16 (on the capture thread, which then exits), the watcher starts four seconds later and can never open joystick 0 again; on the next launch the yoke works all evening, until 21:52:30 when they open settings once, and after ~40 s of failures they give up and rebind PTT to a mouse side button. So the symptom was "**the yoke stops working as soon as you set it up**", which reads as a bad device, not as a threading bug. `PttWatcher._start_joystick()` and `PttCapture.start()` now do the init on the calling (GUI) thread and hand the module to the worker; the workers never touch initialisation. Pinned by `SdlInitThreadTest`, which spies on which thread `_import_pygame()` runs on.
