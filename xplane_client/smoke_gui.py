"""GUI 冒烟测试：离屏把窗口都建起来，不连服务器、不连模拟器、不开音频设备。
    ...
    python smoke_gui.py        （在 xplane_client 目录下运行）
...
class FakeRadioClient:
    ...
    def __init__(self):
        ...
        self.freq = 118.0
        self.xplane = types.SimpleNamespace(
            addr=("127.0.0.1", 49000),
            read_com1_freq=lambda addr: self.freq)
...
def main():
    ...
    def keeps_last_frequency():
        """X-Plane 是 UDP，丢一两个包很正常，显示不该跟着闪。"""
        client.freq = None
        main_window.update_frequency()
        assert "118.000" in main_window.freq_label.text(), main_window.freq_label.text()
        client.xplane.read_com1_freq = lambda addr: (_ for _ in ()).throw(
            OSError("模拟器没开"))
        main_window.update_frequency()
        assert "118.000" in main_window.freq_label.text(), main_window.freq_label.text()
        client.freq = 121.7
        client.xplane.read_com1_freq = lambda addr: client.freq
    check("丢包时保留上一次频率", keeps_last_frequency)

    def placeholder_before_the_first_read():
        fresh = gui.MainWindow(FakeRadioClient())
        fresh.radio_client.xplane.read_com1_freq = lambda addr: (
            _ for _ in ()).throw(OSError("模拟器没开"))
        fresh.update_frequency()
        assert "-.---" in fresh.freq_label.text(), fresh.freq_label.text()
        fresh.timer.stop()
    check("一次都没读到时显示占位符", placeholder_before_the_first_read)
