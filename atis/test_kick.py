"""被服务端踢下线之后不许连回去。
    ...
    python -m unittest test_kick -v      （在 atis 目录下运行）

自带替身：numpy / pyaudio / pymumble / opuslib 全部在导入 voice 之前换掉，所以
开发机上不装原生库（尤其是 opus）也能跑。test_atis.py 用的是真的 numpy 和
pymumble，在没有那些库的机器上根本导入不了，那正是这套逻辑需要一个跑得起来的
家的原因。
...
通播这边多一条：每个席位各有一条连接，所以被踢只停这一个席位，别的还在播。
"""
...
# 生；但在没装的机器上，留着的替身会漏给 test_atis.py 之类真的需要 numpy 的模
...
_STUBS = ("numpy", "pyaudio", "pymumble_py3", "pymumble_py3.constants",
          ...
          "pyttsx3", "requests", "scipy", "scipy.signal")
...
broadcast = _import_under_stubs("broadcast")
...
class BoundedReconnectKickTest(unittest.TestCase):
    ...
    def make(self):
        ...
        bounded = type("Bounded", (broadcast.BoundedReconnect, FakeBase), {})
    ...
    def test_a_kicked_connection_never_dials_again(self):
        ...
        self.assertEqual(result, broadcast.PYMUMBLE_CONN_STATE_FAILED)
...
class UserRemovedTest(unittest.TestCase):
    ...
    def make(self):
        client = broadcast.Broadcaster.__new__(broadcast.Broadcaster)
        ...
        client.station = types.SimpleNamespace(callsign="ZSPD_ATIS")
    ...
    def test_a_kick_stops_this_station_only(self):
        """停的是这个席位。通播一个窗口里跑好几个席位，各有各的连接，
        一个被顶掉不该把别的一起停了。"""
        client = self.make()
        client._on_user_removed({"session": 42},
                                {"session": 42, "actor": 1, "reason": "x"})
        self.assertTrue(client.stop_event.is_set(), "这个席位要停播")
