...
报"连不上"的时候手里一点线索都没有。所以统一写成日志文件：

    .\log\YYYY-MM-DD.log      按日期滚动保存
...
def setup(debug=False):
    ...
    # 写入 .\log\YYYY-MM-DD.log
    if _ensure_log_dir():
        fname = _log_filename()
        _log_path = os.path.abspath(os.path.join(LOG_DIR, fname))
        try:
            file_handler = logging.handlers.RotatingFileHandler(
                _log_path, maxBytes=4 * 1024 * 1024, backupCount=3,
                encoding="utf-8")
        ...
        _log_path = None
        print("无法创建日志目录 ./log/", file=sys.stderr)
