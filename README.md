...
`voice.py`、`traffic.py`、`chime.py`、`observer.py` 必须逐字节一致，有测试盯着。

`xpc/` 和 `msfs/` 还有一个**观察员模式**（连接栏上的勾），给双人机组的右座用：
只连语音、不连 FSD，所以网络上不会多出一架飞机，也不会有重复呼号被踢。频率跟着
自己的 COM1 走，没开模拟器就在无线电栏里手输一个。代价是收不到管制的文字消息
——那是发给机长呼号的。两个人要用各自的账号。
